...
void GameUI::render_menu()
{
    ...
    solo.setFillColor(menu_selection == 0 ? sf::Color::Yellow
                                          : sf::Color::White);
...
void GameUI::render(const GameState &state)
{
    ...
    for (int y = 0; y < BOARD_SIZE; ++y)
    {
        for (int x = 0; x < BOARD_SIZE; ++x)
        {
            if (state.board.get(y, x) == BLACK)
            {
                ...
                stone.setPosition({(float)(MARGIN + x * CELL_SIZE),
                                   (float)(MARGIN + y * CELL_SIZE)});
                window.draw(stone);
            }
        ...
        current_state == UIState::PLAYING_MULTI)
    {
        ...
        // Check Win conditions (5 in a row OR 10 captures)
        if (!state.game_over)
        {
            bool black_wins =
                Rules::check_win_condition(state, Player::BLACK) ||
            ...
            {
                // Will set game_over = true in logic loop, but UI can reflect
                // immediately We probably should mutate this in Input or logic
                // step, mutating `state` here requires const_cast or mutable
                // Better: let Logic set it. Since this is just UI, we check:
            }
        }
        ...
        if (state.game_over)
        {
            ...
            win_msg.setPosition({window.getSize().x / 2.0f -
                                     win_msg.getGlobalBounds().size.x / 2.0f,
            ...
            btn.setPosition({window.getSize().x / 2.0f - 100,
            ...
            replay_txt.setPosition(
                {window.getSize().x / 2.0f -
                     replay_txt.getGlobalBounds().size.x / 2.0f,
