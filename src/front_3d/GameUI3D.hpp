...
private:
  ...
  static constexpr float STONE_RADIUS =
      0.35f; // Rayon des sphères (unités monde)
  ...
  // Même principe qu'en 2D (Input.cpp) : un alignement de 5 ne gagne que si
  // l'adversaire ne peut pas immédiatement capturer une paire de cet
  // alignement. Si c'est possible, on lui donne UNE chance avant de valider la
  // victoire.
  bool pending_alignment_win_;
  Player pending_winner_;

  // ── Thread IA (Solo uniquement) ───────────────────────────────────────────
  // getBestMove() est bloquant (~500ms). On le lance dans un thread séparé
  // pour que la caméra reste libre pendant la réflexion de l'IA.
  ...
  void
  update_camera_rotation(); // Lit GetMouseDelta(), met à jour yaw/pitch/target
  ...
  void handle_ai_turn(GameState &state, Gomoku &gomoku); // Lance le thread IA
  void
  apply_win_check(GameState &state); // Endgame capture complète (comme le 2D)
